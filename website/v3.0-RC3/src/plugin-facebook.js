...
;
plugin.extend("facebook", {name: "", version: "", userInfo: null, HttpMethod: {"Get": "get", "Post": "post", "Delete": "delete"}, CodeSucceed: 0, ctor: function (config) {
    this.name = "facebook";
    this.version = "1.0";
    this.userInfo = {};
    if (!FB)return;
    ...
    FB.login(function (response) {
        if (response["authResponse"]) {
            self.userInfo = response["authResponse"];
            typeof callback === "function" && callback(0, {accessToken: response["authResponse"]["accessToken"]})
        } else typeof callback === "function" && callback(response["error_code"] || 1, {error_message: "unknown"})
    }, {scope: "publish_actions"})
}, isLoggedIn: function (callback) {
    var self = this;
    FB.getLoginStatus(function (response) {
        if (response)if (response["status"] === "connected") {
            self.userInfo = response["authResponse"];
            typeof callback ===
            "function" && callback(0, {isLoggedIn: true, accessToken: response["authResponse"]["accessToken"]})
        } else typeof callback === "function" && callback(0, {isLoggedIn: false, accessToken: ""}); else typeof callback === "function" && callback(1, {error_message: "Unknown"})
    })
    ...
    FB.logout(function (response) {
        ...
        } else typeof callback === "function" && callback(response["error_code"] || 1, {error_message: response["error_message"] ||
            "Unknown"})
    })
    ...
    FB.login(function (response) {
        if (response["authResponse"]) {
            ...
            self.userInfo = response["authResponse"];
            typeof callback === "function" && callback(0, {permissions: permissList})
        } else typeof callback === "function" && callback(response["error_code"] || 1, {error_message: response["error_message"] || "Unknown"})
    }, {scope: permissionsStr, return_scopes: true})
},
    requestAccessToken: function (callback) {
        if (typeof callback !== "function")return;
        if (this.userInfo.accessToken)callback(0, {accessToken: this.userInfo["accessToken"]}); else {
            var self = this;
            FB.getLoginStatus(function (response) {
                if (response && response["status"] === "connected") {
                    self.userInfo = response["authResponse"];
                    callback(0, {accessToken: response["authResponse"]["accessToken"]})
                } else callback(response["error_code"] || 1, {error_message: response["error_message"] || "Unknown"})
            })
        }
        ...
        FB.ui({method: "share",
            ...
            if (response)if (response["post_id"])typeof callback === "function" && callback(0, {didComplete: true, post_id: response["post_id"]}); else typeof callback === "function" && callback(response.error_code || 1, {error_message: "Unknown"}); else typeof callback === "function" && callback(1, {error_message: "Unknown"})
        })
        ...
        if (info["method"] == "share_open_graph" && info["url"])if (info["url"]) {
            var obj = {};
            if (info["preview_property"])obj[info["preview_property"]] = info["url"]; else obj["object"] = info["url"];
            for (var p in info)if (p != "method" && p != "action_type" && p != "action_properties") {
                info[p] && (obj[p] = info[p]);
                delete info[p]
            }
            info["action_properties"] = JSON.stringify(obj)
        } else return; else if (!info["href"])return;
        if (info["method"] != "share_open_graph" && info["method"] != "share_link" && info["method"] != "apprequests") {
            cc.log("web is not supported what this it method");
            return
        }
        FB.ui(info, function (response) {
            if (response)if (response["post_id"])typeof callback === "function" && callback(0, {didComplete: true, post_id: response["post_id"]}); else typeof callback === "function" && callback(0, response); else typeof callback === "function" && callback(1, {error_message: "Unknow error"})
        })
        ...
        FB.api(path, httpmethod, params, function (response) {
            ...
            callback(response["error"]["code"], {error_message: response["error"]["message"] || "Unknown"}); else typeof callback === "function" && callback(0, response)
        })
    }, getPermissionList: function (callback) {
        FB.api("/me/permissions", function (response) {
            if (response["data"]) {
                var permissionList = [];
                for (var i = 0; i < response["data"].length; i++)permissionList.push(response["data"][i]["permission"]);
                typeof callback == "function" && callback(0, {permissions: permissionList})
            } else {
                if (!response["error"])response["error"] = {};
                typeof callback ==
                "function" && callback(response["error"]["code"] || 1, {error_message: response["error"]["message"] || "Unknown"})
            }
        })
        ...
        FB.ui(info, function (response) {
            if (response["error_code"])callback(response["error_code"] || 1, {error_message: response["error_message"] || "Unknown"}); else callback(0, response)
        })
    }});