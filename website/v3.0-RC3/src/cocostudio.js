...
ccs.dataReaderHelper = {ConfigType: {DragonBone_XML: 0, CocoStudio_JSON: 1, CocoStudio_Binary: 2}, _configFileList: [], _flashToolVersion: ccs.CONST_VERSION_2_0, _positionReadScale: 1, _asyncRefCount: 0, _asyncRefTotalCount: 0, _dataQueue: null, setPositionReadScale: function (scale) {
    ...
    decodeArmatureFromJSON: function (json, dataInfo) {
        ...
        if (colorTransformXML && colorTransformXML.length > 0) {
            ...
            alpha = parseFloat(colorTransformXML.getAttribute(ccs.CONST_A_ALPHA)) ||
                0;
            ...
            blueOffset = parseFloat(colorTransformXML.getAttribute(ccs.CONST_A_BLUE_OFFSET)) ||
                0;
        ...
        var bd_src = json[ccs.CONST_A_BLEND_SRC] == null ?
            cc.BLEND_SRC : json[ccs.CONST_A_BLEND_SRC];
        ...
        if (dataInfo.cocoStudioVersion < ccs.CONST_VERSION_COMBINED)frameData.duration = json[ccs.CONST_A_DURATION] == null ? 1 : json[ccs.CONST_A_DURATION];
        ...
        for (var i = 0; i < contoursXML.length; i++)textureData.addContourData(this.decodeContour(contoursXML[i], dataInfo));
        return textureData
        ...
        var autoLoad = dataInfo.asyncStruct ==
            null ? ccs.armatureDataManager.isAutoLoadSpriteFile() : dataInfo.asyncStruct.autoLoadSpriteFile;
        if (autoLoad) {
            ...
            for (i = 0; i < configFiles.length; i++) {
                ...
                ccs.armatureDataManager.addSpriteFrameFromFile(locPlistPath,
                    locImagePath, dataInfo.filename)
            }
        }
        armatureData = null;
        animationData = null
        ...
        node.scaleY = json[ccs.CONST_A_SCALE_Y] ==
            null ? 1 : json[ccs.CONST_A_SCALE_Y];
        var colorDic;
        if (dataInfo.cocoStudioVersion < ccs.VERSION_COLOR_READING) {
            ...
            if (colorDic) {
                node.a =
                        colorDic[ccs.CONST_A_ALPHA] == null ? 255 : colorDic[ccs.CONST_A_ALPHA];
                node.r = colorDic[ccs.CONST_A_RED] == null ? 255 : colorDic[ccs.CONST_A_RED];
                node.g = colorDic[ccs.CONST_A_GREEN] == null ? 255 : colorDic[ccs.CONST_A_GREEN];
                node.b = colorDic[ccs.CONST_A_BLUE] == null ? 255 : colorDic[ccs.CONST_A_BLUE];
                node.isUseColorInfo = true
        ...
        if (selector && typeof selector === "function")selector.call(target,
            percent);
...
ccs.TweenFunction.tweenTo = function (time, type, easingParam) {
    ...
    switch (type) {
        ...
        case ccs.TweenType.elasticEaseIn:
            var period = 0.3;
            if (null != easingParam && easingParam.length > 0)period = easingParam[0];
        ...
        case ccs.TweenType.elasticEaseInOut:
            var period = 0.3;
            if (null != easingParam && easingParam.length >
                0)period = easingParam[0];
    ...
    update: function (dt) {
        ...
        for (var i = 0, len = locChildren.length; i < len; i++) {
            ...
            if (selBone && selBone.getDisplayRenderNode) {
                var node =
                    selBone.getDisplayRenderNode();
                if (null == node)continue;
                if (cc._renderType === cc._RENDER_TYPE_WEBGL)node.setShaderProgram(this._shaderProgram);
                switch (selBone.getDisplayRenderNodeType()) {
                    case ccs.DISPLAY_TYPE_SPRITE:
                        if (node instanceof ccs.Skin)if (cc._renderType === cc._RENDER_TYPE_WEBGL) {
                            node.updateTransform();
                            var func = selBone.getBlendFunc();
                            if (func.src != alphaPremultiplied.src || func.dst != alphaPremultiplied.dst)node.setBlendFunc(selBone.getBlendFunc()); else if (this._blendFunc.src == alphaPremultiplied.src &&
                                this._blendFunc.dst == alphaPremultiplied.dst && !node.getTexture().hasPremultipliedAlpha())node.setBlendFunc(alphaNonPremultipled); else node.setBlendFunc(this._blendFunc);
                            node.draw(ctx)
                        } else node.visit(ctx);
                        break;
                    case ccs.DISPLAY_TYPE_ARMATURE:
                ...
                if (cc._renderType === cc._RENDER_TYPE_WEBGL)selBone.setShaderProgram(this._shaderProgram);
                selBone.visit(ctx)
            }
        }
    }, onEnter: function () {
...
ccs.ActionRotationFrame = ccs.ActionFrame.extend({_rotation: 0, ctor: function () {
    ...
    if (srcFrame === undefined)return this._getEasingAction(cc.rotateTo(duration, this._rotation)); else if (!(srcFrame instanceof cc.ActionRotationFrame))return this.getAction(duration); else {
        var diffRotation =
            this._rotation - srcFrame._rotation;
...
ccs.ActionTintFrame = ccs.ActionFrame.extend({_color: null, ctor: function () {
    ...
    return this._getEasingAction(cc.tintTo(duration, this._color.r, this._color.g,
        this._color.b))
}});
...
ccs.ActionNode = ccs.Class.extend({_currentFrameIndex: 0, _destFrameIndex: 0, _unitTime: 0, _actionTag: 0, _bject: null, _actionSpawn: null, _action: null, _frameArray: null, _frameArrayNum: 0, ctor: function () {
    ...
    else if (this._object instanceof ccui.Widget)return this._object;
    return null
    ...
    for (var i = 0; i < this._frameArrayNum; i++) {
        ...
        for (var j = 0; j < locArray.length; j++) {
            ...
            if (j != 0) {
                ...
                var locAction =
                    locFrame.getAction(locDuration);
