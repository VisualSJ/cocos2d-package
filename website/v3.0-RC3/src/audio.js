if (cc.sys._supportWebAudio) {
    ...
    cc.WebAudio = cc.Class.extend({_events: null, _buffer: null, _sourceNode: null, _volumeNode: null, src: null, preload: null, autoplay: null, controls: null, mediagroup: null, currentTime: 0, startTime: 0, duration: 0, _loop: null, _volume: 1, _pauseTime: 0, _paused: false, _stopped: true, _loadState: -1, ctor: function (src) {
        ...
        if (!sourceNode["playbackState"])sourceNode["onended"] =
            function () {
                this._stopped = true
            };
        self._paused = false;
        ...
        if (!self._stopped && sourceNode && (sourceNode["playbackState"] == 2 || !sourceNode._stopped))return;
        ...
        var self = this;
        if (self._paused) {
            var offset = self._buffer ? (self._pauseTime - self.startTime) % self._buffer.duration :
                0;
            this._play(offset)
        }
    }, stop: function () {
    ...
    cc.defineGetterSetter(_p, "ended", function () {
        ...
        if (sourceNode["playbackState"] == null)return sourceNode._stopped; else return sourceNode["playbackState"] == 3
    });
    _p.played;
    cc.defineGetterSetter(_p, "played", function () {
        var sourceNode = this._sourceNode;
        return sourceNode && (sourceNode["playbackState"] ==
            2 || !sourceNode._stopped)
    })
}
cc.AudioEngine = cc.Class.extend({_soundSupported: false, _currMusic: null, _currMusicPath: null, _musicPlayState: 0, _audioID: 0, _effects: {}, _audioPool: {}, _effectsVolume: 1, _maxAudioInstance: 5, _effectPauseCb: null, _playings: [], ctor: function () {
    ...
    } else if (url != self._currMusicPath) {
        audio = self._getAudioByUrl(url);
    ...
    if (!audio.ended)if (audio.stop)audio.stop(); else {
        audio.pause();
    ...
    if (audio && !audio.ended) {
        if (audio.stop)audio.stop(); else {
            audio.pause();
            if (audio.readyState > 2 && audio.duration && audio.duration != Infinity)audio.currentTime = audio.duration
        }
        return true
    }
    return false
    ...
    if (cc.sys.isMobile && cc.sys.os == cc.sys.OS_IOS)audio = this._getEffectAudio(effList, url); else {
    ...
    _getEffectAudio: function (effList, url) {
        ...
        if (this._effectPauseCb)cc._addEventListener(audio, "pause", this._effectPauseCb);
...
if (!cc.sys._supportWebAudio && cc.sys._supportMultipleAudio < 0)cc.AudioEngineForSingle = cc.AudioEngine.extend({_waitingEffIds: [], _pausedEffIds: [], _currEffect: null, _maxAudioInstance: 2, _effectCache4Single: {}, _needToResumeMusic: false, _expendTime4Music: 0, _isHiddenMode: false, _playMusic: function (audio) {
    ...
    pauseEffect: function (effectId) {
        ...
        for (var key in sglCache) {
            ...
            if (eff.readyState > 2 && eff.duration && eff.duration != Infinity)eff.currentTime =
                eff.duration
        }
        waitings.length = 0;
        pauseds.length = 0;
        for (var key in audioPool) {
            ...
            for (var i = 0, li = list.length; i < li; i++) {
                var eff = list[i];
                eff.loop = false;
                if (eff.readyState > 2 && eff.duration && eff.duration != Infinity)eff.currentTime = eff.duration
            }
        }
        ...
        if (currEffect)if (currEffect._isToPlay) {
            ...
            if (currMusic.readyState > 2 && currMusic.duration && currMusic.duration != Infinity) {
        ...
        while (true) {
            ...
            if (eff._isToPlay || eff.loop || eff.duration && eff.currentTime + expendTime < eff.duration) {
                ...
                eff._isToPlay = false;
                return eff
            } else if (eff.readyState > 2 && eff.duration && eff.duration != Infinity)eff.currentTime = eff.duration
        }
...
cc._audioLoader = {_supportedAudioTypes: null, getBasePath: function () {
    ...
    var _Audio;
    if (typeof window["cc"] != "object" && cc.sys.browserType == "firefox")_Audio = Audio; else _Audio = location.origin == "file://" ? Audio : cc.WebAudio || Audio;
