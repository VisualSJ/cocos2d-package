...
sp.Skeleton = cc.Node.extend({_skeleton: null, _rootBone: null, _timeScale: 1, _debugSlots: false, _debugBones: false, _premultipliedAlpha: false, _ownsSkeletonData: null, _atlas: null, _blendFunc: null, ctor: function (skeletonDataFile, atlasFile, scale) {
    ...
    for (var i = 0, slotCount = slots.length; i < slotCount; ++i) {
        var slot = slots[i];
        if (!slot.attachment || slot.attachment.type !=
            sp.ATTACHMENT_TYPE.REGION)continue;
        ...
        maxX = Math.max(maxX, vertices[VERTEX.X1] * scaleX, vertices[VERTEX.X4] * scaleX, vertices[VERTEX.X2] *
            scaleX, vertices[VERTEX.X3] * scaleX);
    ...
    isOpacityModifyRGB: function () {
        ...
        if (cc._renderType === cc._RENDER_TYPE_CANVAS) {
            ...
            for (var i = 0, n = drawOrder.length; i < n; i++) {
                ...
                var bone = slot.bone;
                selSprite.setPosition(bone.worldX + attachment.x * bone.m00 + attachment.y * bone.m01, bone.worldY + attachment.x * bone.m10 + attachment.y *
                    bone.m11);
        ...
        var additive, textureAtlas,
            attachment, slot, i, n, quad = new cc.V3F_C4B_T2F_Quad;
        var locBlendFunc = this._blendFunc;
        ...
        if (this._debugSlots) {
            ...
            for (i = 0, n = locSkeleton.slots.length; i < n; i++) {
                ...
                points.push(cc.p(quad.tr.vertices.x,
                    quad.tr.vertices.y));
        ...
        if (this._debugBones) {
            ...
            drawingUtil.setDrawColor(0, 0, 255, 255);
            for (i =
                     0, n = locSkeleton.bones.length; i < n; i++) {
                bone = locSkeleton.bones[i];
...
sp.SkeletonAnimation = sp.Skeleton.extend({_state: null, _target: null, _callback: null, init: function () {
    ...
    this._animationStateCallback(trackIndex,
        sp.ANIMATION_EVENT_TYPE.EVENT, event, 0)
