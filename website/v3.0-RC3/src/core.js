...
cc.loader = {_jsCache: {}, _register: {}, _langPathCache: {}, _aliases: {}, resPath: "", audioPath: "", cache: {}, getXMLHttpRequest: function () {
    ...
    if (navigator.userAgent.indexOf("Trident/5") > -1)self._loadJs4Dependency(preDir, list, 0, callback); else cc.async.map(list, function (item, index, cb1) {
        var jsPath = cc.path.join(preDir,
            item);
    ...
    _loadJsImg: function () {
        ...
        if (!cc._isNodeJs) {
            ...
            if (/msie/i.test(navigator.userAgent) && !/opera/i.test(navigator.userAgent)) {
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4)xhr.status == 200 ? cb(null, xhr.responseText) : cb(errInfo)
                }
            } else {
        ...
        if (cb !== undefined)opt.isCrossOrigin = option.isCrossOrigin == null ? opt.isCrossOrigin : option.isCrossOrigin; else if (option !== undefined)cb = option;
        var img = new Image;
        ...
        if (len == 3) {
            if (typeof option == "function")if (typeof cb == "function")option = {trigger: option, cb: cb}; else option = {cb: option, cbTarget: cb}
        } else if (len == 2) {
...
cc._initSys = function (config, CONFIG_KEY) {
    ...
    if (nav.appVersion.indexOf("Win") != -1)osName = sys.OS_WINDOWS; else if (iOS)osName = sys.OS_IOS; else if (nav.appVersion.indexOf("Mac") != -1)osName = sys.OS_OSX; else if (nav.appVersion.indexOf("X11") != -1)osName = sys.OS_UNIX; else if (isAndroid)osName = sys.OS_ANDROID; else if (nav.appVersion.indexOf("Linux") != -1)osName = sys.OS_LINUX;
    sys.os = osName;
...
cc._setup = function (el, width, height) {
    ...
    if (cc.sys.os === cc.sys.OS_IOS && cc.sys.browserType ===
        ...
        win.cancelAnimationFrame = window.cancelAnimationFrame || window.cancelRequestAnimationFrame || window.msCancelRequestAnimationFrame ||
            window.mozCancelRequestAnimationFrame || window.oCancelRequestAnimationFrame || window.webkitCancelRequestAnimationFrame || window.msCancelAnimationFrame || window.mozCancelAnimationFrame || window.webkitCancelAnimationFrame || window.oCancelAnimationFrame || ctTime
    }
    ...
    if (element.tagName == "CANVAS") {
        ...
        localCanvas.parentNode.insertBefore(localContainer,
            localCanvas);
...
cc.game = {DEBUG_MODE_NONE: 0, DEBUG_MODE_INFO: 1, DEBUG_MODE_WARN: 2, DEBUG_MODE_ERROR: 3, DEBUG_MODE_INFO_FOR_WEB_PAGE: 4, DEBUG_MODE_WARN_FOR_WEB_PAGE: 5, DEBUG_MODE_ERROR_FOR_WEB_PAGE: 6, EVENT_HIDE: "game_on_hide", EVENT_SHOW: "game_on_show", _eventHide: null, _eventShow: null, _onBeforeStartArr: [], CONFIG_KEY: {engineDir: "engineDir", dependencies: "dependencies", debugMode: "debugMode", showFPS: "showFPS", frameRate: "frameRate", id: "id", renderMode: "renderMode", jsList: "jsList", classReleaseMode: "classReleaseMode"}, _prepareCalled: false,
    _prepared: false, _paused: true, _intervalId: null, config: null, onStart: null, onStop: null, setFrameRate: function (frameRate) {
        ...
        var _run = function () {
            ...
            if (cc._supportRender)self._checkPrepare = setInterval(function () {
                if (self._prepared) {
        ...
        var _init = function (cfg) {
            ...
            if (cfg[CONFIG_KEY.debugMode] == null)cfg[CONFIG_KEY.debugMode] =
                0;
        ...
        var self = this;
        var config = self.config, CONFIG_KEY = self.CONFIG_KEY, engineDir =
            config[CONFIG_KEY.engineDir], loader = cc.loader;
...
cc.color = function (r, g, b, a) {
    ...
    if (typeof r === "object")return{r: r.r, g: r.g, b: r.b, a: r.a == null ? 255 : r.a};
...
cc._Dictionary = cc.Class.extend({_keyMapTb: null, _valueMapTb: null, __currId: 0, ctor: function () {
    ...
    for (var keyId in locKeyMapTb)if (locKeyMapTb[keyId] ===
        key)return this._valueMapTb[keyId];
    return null
...
cc.inputManager = {_mousePressed: false, _isRegisterEvent: false, _preTouchPoint: cc.p(0, 0), _prevMousePoint: cc.p(0, 0), _preTouchPool: [], _preTouchPoolPointer: 0, _touches: [], _touchesIntegerDict: {}, _indexBitsUsed: 0, _maxTouches: 5, _accelEnabled: false, _accelInterval: 1 / 30, _accelMinus: 1, _accelCurTime: 0, _acceleration: null, _accelDeviceEvent: null, _getUnUsedIndex: function () {
    ...
    for (var i = 0, len = touches.length; i < len; i++) {
        ...
        if (index == null) {
            ...
            curTouch =
                this._touches[unusedIndex] = new cc.Touch(selTouch._point.x, selTouch._point.y, selTouch.getID());
    ...
    var win = window;
    var box = null;
    if (typeof element.getBoundingClientRect === "function")box = element.getBoundingClientRect(); else if (element instanceof HTMLCanvasElement)box = {left: 0, top: 0, width: element.width, height: element.height}; else box = {left: 0, top: 0,
        width: parseInt(element.style.width), height: parseInt(element.style.height)};
    ...
    var touch =
        new cc.Touch(location.x, location.y);
    ...
    getPointByEvent: function (event, pos) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (touch_event) {
                var location;
                if (cc.sys.BROWSER_TYPE_FIREFOX ===
                    cc.sys.browserType)location = locView.convertToLocationInView(touch_event.pageX, touch_event.pageY, pos); else location = locView.convertToLocationInView(touch_event.clientX, touch_event.clientY, pos);
        ...
        if (supportMouse) {
            ...
            cc._addEventListener(element, "mousemove",
                function (event) {
                    var pos = selfPointer.getHTMLElementPosition(element);
                    var location = selfPointer.getPointByEvent(event, pos);
            ...
            cc._addEventListener(element, "DOMMouseScroll", function (event) {
                var pos = selfPointer.getHTMLElementPosition(element);
                var location =
                    selfPointer.getPointByEvent(event, pos);
                var mouseEvent = selfPointer.getMouseEvent(location, pos, cc.EventMouse.SCROLL);
                mouseEvent.setButton(event.button);
        ...
        if (supportTouches) {
            cc._addEventListener(element, "touchstart",
                function (event) {
            ...
            cc._addEventListener(element, "touchcancel",
                function (event) {
                    if (!event.changedTouches)return;
                    var pos = selfPointer.getHTMLElementPosition(element);
                    pos.left -= document.body.scrollLeft;
...
cc.eventManager = {DIRTY_NONE: 0, DIRTY_FIXED_PRIORITY: 1 << 0, DIRTY_SCENE_GRAPH_PRIORITY: 1 << 1, DIRTY_ALL: 3, _listenersMap: {}, _priorityDirtyFlagMap: {}, _nodeListenersMap: {}, _nodePriorityMap: {}, _globalZOrderNodeMap: {}, _toAddedListeners: [], _dirtyNodes: [], _inDispatch: 0, _isEnabled: false, _nodePriorityIndex: 0, _internalCustomListenerIDs: [cc.game.EVENT_HIDE, cc.game.EVENT_SHOW], _setDirtyForNode: function (node) {
    ...
    _updateDirtyFlagForSceneGraph: function () {
        ...
        if (!l1 || !l2 || !l1._getSceneGraphPriority() || !l2._getSceneGraphPriority())return-1;
        ...
        if (fixedPriorityListeners)for (i = 0; i < fixedPriorityListeners.length;) {
            ...
            if (!selListener._isRegistered())cc.arrayRemoveObject(fixedPriorityListeners, selListener); else++i
        }
        ...
        var eventCode = cc.EventTouch.EventCode, event =
            callbackParams.event, touches = callbackParams.touches, getCode = event.getEventCode();
        event._setCurrentTarget(listener._node);
        if (getCode == eventCode.BEGAN && listener.onTouchesBegan)listener.onTouchesBegan(touches, event); else if (getCode == eventCode.MOVED && listener.onTouchesMoved)listener.onTouchesMoved(touches, event); else if (getCode == eventCode.ENDED && listener.onTouchesEnded)listener.onTouchesEnded(touches, event); else if (getCode == eventCode.CANCELLED && listener.onTouchesCancelled)listener.onTouchesCancelled(touches,
            event);
        if (event.isStopped()) {
            cc.eventManager._updateListeners(event);
            return true
        ...
        if (listeners) {
            ...
            if (listeners.length ===
                0)delete this._nodeListenersMap[node.__instanceId]
        }
        ...
        } else if (locNodeListenersMap[node.__instanceId] !=
            null) {
            if (!locGlobalZOrderNodeMap[node.getGlobalZOrder()])locGlobalZOrderNodeMap[node.getGlobalZOrder()] = [];
            locGlobalZOrderNodeMap[node.getGlobalZOrder()].push(node.__instanceId)
        }
        ...
        var listener = cc._EventListenerCustom.create(eventName,
            callback);
        ...
        for (var selKey in locListener) {
            ...
            if (isFound)this._setDirty(listener._getListenerID(), this.DIRTY_SCENE_GRAPH_PRIORITY);
            else {
        ...
        if (!isFound) {
            ...
            for (var i = 0, len = locToAddedListeners.length; i < len; i++) {
                ...
                if (selListener == listener) {
                    cc.arrayRemoveObject(locToAddedListeners,
                        selListener);
                    break
                }
            }
...
cc.Node = cc.Class.extend({_localZOrder: 0, _globalZOrder: 0, _vertexZ: 0, _rotationX: 0, _rotationY: 0, _scaleX: 1, _scaleY: 1, _position: null, _skewX: 0, _skewY: 0, _children: null, _visible: true, _anchorPoint: null, _anchorPointInPoints: null, _contentSize: null, _running: false, _parent: null, _ignoreAnchorPointForPosition: false, tag: cc.NODE_TAG_INVALID, userData: null, userObject: null, _transformDirty: true, _inverseDirty: true, _cacheDirty: true, _cachedParent: null, _transformGLDirty: null, _transform: null, _inverse: null, _reorderChildDirty: false,
    ...
    setScale: function (scale, scaleY) {
        ...
        locAPP.x =
            locSize.width * locAnchorPoint.x;
...
cc.textureCache = {_textures: {}, _textureColorsCache: {}, _textureKeySeq: 0 | Math.random() * 1E3, _loadedTexturesBefore: {}, _initializingRenderer: function () {
    ...
    description: function () {
        ...
        for (key in locTextureColorsCache) {
            var selCanvasColorsArr =
                locTextureColorsCache[key];
...
cc.LayerColor = cc.Layer.extend({_blendFunc: null, _className: "LayerColor", getBlendFunc: function () {
    ...
    cc.Layer.prototype.setOpacity.call(this,
        opacity);
    this._updateColor()
}, _blendFuncStr: "source", ctor: null, init: function (color, width, height) {
    ...
    if (cc._renderType === cc._RENDER_TYPE_CANVAS)_t._blendFuncStr =
        cc._getCompositeOperationByBlendFunc(locBlendFunc)
...
if (cc._renderType === cc._RENDER_TYPE_CANVAS) {
    ...
    _p.draw = function (ctx) {
        var context = ctx || cc._renderContext, _t = this;
        if (_t._blendFuncStr != "source")context.globalCompositeOperation = _t._blendFuncStr;
        context.save();
        ...
        var tGradient = context.createLinearGradient(_t._gradientStartPoint.x * scaleX, _t._gradientStartPoint.y * scaleY, _t._gradientEndPoint.x *
            scaleX, _t._gradientEndPoint.y * scaleY);
...
cc.generateTintImageWithMultiply = function (image, color, rect, renderCanvas) {
    ...
    context.fillRect(0, 0, rect.width,
        rect.height);
    context.globalCompositeOperation = "multiply";
...
cc._getCompositeOperationByBlendFunc = function (blendFunc) {
    if (!blendFunc)return"source"; else if (blendFunc.src == cc.SRC_ALPHA && blendFunc.dst == cc.ONE || blendFunc.src == cc.ONE && blendFunc.dst == cc.ONE)return"lighter"; else if (blendFunc.src == cc.ZERO && blendFunc.dst == cc.SRC_ALPHA)return"destination-in"; else if (blendFunc.src == cc.ZERO && blendFunc.dst == cc.ONE_MINUS_SRC_ALPHA)return"destination-out"; else return"source"
};
cc.Sprite = cc.Node.extend({dirty: false, atlasIndex: 0, textureAtlas: null, _batchNode: null, _recursiveDirty: null, _hasChildren: null, _shouldBeHidden: false, _transformToBatch: null, _blendFunc: null, _texture: null, _rect: null, _rectRotated: false, _offsetPosition: null, _unflippedOffsetPositionFromCenter: null, _opacityModifyRGB: false, _flippedX: false, _flippedY: false, _textureLoaded: false, _loadedEventListeners: null, _newTextureWhenChangeColor: null, _className: "Sprite", _oldDisplayColor: cc.color.WHITE, textureLoaded: function () {
    ...
    setVertexRect: function (rect) {
        ...
        if (this._reorderChildDirty) {
            ...
            if (this._batchNode)this._arrayMakeObjectsPerformSelector(_children, cc.Node._StateCallbackType.sortAllChildren);
    ...
    }, _quad: null, _quadWebBuffer: null, _quadDirty: false, _colorized: false, _blendFuncStr: "source", _originalTexture: null, _textureRect_Canvas: null, _drawSize_Canvas: null, ctor: null, _softInit: function (fileName, rect, rotated) {
    ...
    _textureLoadedCallback: null, setTextureRect: null, updateTransform: null, addChild: null, updateColor: function () {
        ...
        return cc.SpriteFrame.create(this._texture, cc.rectPointsToPixels(this._rect),
            this._rectRotated, cc.pointPointsToPixels(this._unflippedOffsetPositionFromCenter), cc.sizePointsToPixels(this._contentSize))
        ...
        if (locTexture && locRect.validRect && this._originalTexture) {
            ...
            if (locElement instanceof HTMLCanvasElement && !this._rectRotated && !this._newTextureWhenChangeColor && this._originalTexture._htmlElementObj != locElement)cc.generateTintImageWithMultiply(this._originalTexture._htmlElementObj, this._displayedColor, locRect,
                locElement); else {
        ...
        var left, right, top, bottom, tempSwap, locQuad =
            this._quad;
...
if (cc._renderType === cc._RENDER_TYPE_CANVAS) {
    ...
    _p.setBlendFunc = function (src, dst) {
        ...
        if (cc._renderType === cc._RENDER_TYPE_CANVAS)_t._blendFuncStr = cc._getCompositeOperationByBlendFunc(locBlendFunc)
    };
    _p.init = function () {
        var _t = this;
    ...
    _p._textureLoadedCallback = function (sender) {
        ...
        if (!locRect)locRect =
            cc.rect(0, 0, sender.width, sender.height); else if (cc._rectEqualToZero(locRect)) {
    ...
    _p.setTextureRect = function (rect, rotated, untrimmedSize) {
        ...
        _t.setContentSize(untrimmedSize ||
            rect);
    ...
    _p.updateTransform = function () {
        ...
        if (_t.dirty) {
            ...
            if (!_t._visible || locParent && locParent != _t._batchNode && locParent._shouldBeHidden)_t._shouldBeHidden = true; else {
                ...
                if (!locParent || locParent == _t._batchNode)_t._transformToBatch =
                    _t.nodeToParentTransform(); else _t._transformToBatch = cc.affineTransformConcat(_t.nodeToParentTransform(), locParent._transformToBatch)
            }
            _t._recursiveDirty = false;
            _t.dirty = false
        }
        if (_t._hasChildren)_t._arrayMakeObjectsPerformSelector(_t._children, cc.Node._StateCallbackType.updateTransform)
    };
    ...
    _p.setSpriteFrame = function (newFrame) {
        ...
        _t._unflippedOffsetPositionFromCenter.y =
            frameOffset.y;
        ...
        if (pNewTexture != _t._texture)_t.texture =
            pNewTexture;
    ...
    _p.setTexture = function (texture) {
        ...
        cc.assert(!texture || texture instanceof
            cc.Texture2D, cc._LogInfos.CCSpriteBatchNode_setTexture);
    ...
    _p.draw = function (ctx) {
        ...
        var locRect =
            _t._rect, locContentSize = _t._contentSize, locOffsetPosition = _t._offsetPosition, locDrawSizeCanvas = _t._drawSize_Canvas;
        ...
        if (cc.SPRITE_DEBUG_DRAW === 1 || _t._showNode) {
            ...
            var vertices1 = [cc.p(flipXOffset, flipYOffset), cc.p(flipXOffset +
                locRect.width, flipYOffset), cc.p(flipXOffset + locRect.width, flipYOffset - locRect.height), cc.p(flipXOffset, flipYOffset - locRect.height)];
            ...
            cc._drawingUtil.drawPoly(vertices2,
                4, true)
        }
...
cc.SpriteFrame = cc.Class.extend({_offset: null, _originalSize: null, _rectInPixels: null, _rotated: false, _rect: null, _offsetInPixels: null, _originalSizeInPixels: null, _texture: null, _textureFilename: "", _textureLoaded: false, _eventListeners: null, ctor: function (filename, rect, rotated, offset, originalSize) {
    ...
    setOffset: function (offsets) {
        ...
        if (texture &&
            ...
            if (_x > texture.getPixelsWide())cc.error(cc._LogInfos.RectWidth, texture.url);
            if (_y > texture.getPixelsHigh())cc.error(cc._LogInfos.RectHeight, texture.url)
        }
...
cc.Director = cc.Class.extend({_landscape: false, _nextDeltaTimeZero: false, _paused: false, _purgeDirectorInNextLoop: false, _sendCleanupToScene: false, _animationInterval: 0, _oldAnimationInterval: 0, _projection: 0, _accumDt: 0, _contentScaleFactor: 1, _displayStats: false, _deltaTime: 0, _frameRate: 0, _FPSLabel: null, _SPFLabel: null, _drawsLabel: null, _winSizeInPoints: null, _lastUpdate: null, _nextScene: null, _notificationNode: null, _openGLView: null, _scenesStack: null, _projectionDelegate: null, _runningScene: null, _frames: 0, _totalFrames: 0,
    ...
    }, setOpenGLView: null, setProjection: null,
    setViewport: null, getOpenGLView: null, getProjection: null, setAlphaBlending: null, _showStats: function () {
        this._frames++;
...
cc.LabelTTF = cc.Sprite.extend({_dimensions: null, _hAlignment: cc.TEXT_ALIGNMENT_CENTER, _vAlignment: cc.VERTICAL_TEXT_ALIGNMENT_TOP, _fontName: null, _fontSize: 0, _string: "", _originalText: null, _isMultiLine: false, _fontStyleStr: null, _shadowEnabled: false, _shadowOffset: null, _shadowOpacity: 0, _shadowBlur: 0, _shadowColorStr: null, _strokeEnabled: false, _strokeColor: null, _strokeSize: 0, _strokeColorStr: null, _textFillColor: null, _fillColorStr: null, _strokeShadowOffsetX: 0, _strokeShadowOffsetY: 0, _needUpdateTexture: false, _labelCanvas: null,
    _labelContext: null, _lineWidths: null, _className: "LabelTTF", initWithString: function (label, fontName, fontSize, dimensions, hAlignment, vAlignment) {
        var strInfo;
        ...
        this._lineWidths = [];
        this._setColorsString();
        if (fontName && fontName instanceof cc.FontDefinition)this.initWithStringAndTextDefinition(text, fontName); else cc.LabelTTF.prototype.initWithString.call(this, text, fontName, fontSize, dimensions, hAlignment, vAlignment)
    }, init: function () {
