...
cc.TransitionJumpZoom = cc.TransitionScene.extend({ctor: function (t, scene) {
    ...
    var scaleIn = cc.scaleTo(this._duration /
        4, 1);
...
cc.TransitionMoveInL = cc.TransitionScene.extend({ctor: function (t, scene) {
    ...
    return cc.moveTo(this._duration,
        cc.p(0, 0))
...
cc.TransitionShrinkGrow = cc.TransitionScene.extend({ctor: function (t, scene) {
    ...
    this._outScene.runAction(cc.sequence(this.easeActionWithAction(scaleOut),
...
cc.TransitionFade = cc.TransitionScene.extend({_color: null, ctor: function (t, scene, color) {
    ...
    var a = cc.sequence(cc.fadeIn(this._duration / 2), cc.callFunc(this.hideOutShowIn, this), cc.fadeOut(this._duration /
        2), cc.callFunc(this.finish, this));
    f.runAction(a)
}, onExit: function () {
...
cc.TransitionTurnOffTiles = cc.TransitionScene.extend({ctor: function (t, scene) {
    ...
    this._outScene.runAction(cc.sequence(action,
...
cc.TransitionSplitCols = cc.TransitionScene.extend({ctor: function (t, scene) {
    ...
    var seq = cc.sequence(split, cc.callFunc(this.hideOutShowIn, this), split.reverse());
    this.runAction(cc.sequence(this.easeActionWithAction(seq), cc.callFunc(this.finish, this), cc.stopGrid()))
}, easeActionWithAction: function (action) {
    return new cc.EaseInOut(action,
        3)
}, action: function () {
...
cc.TransitionPageTurn = cc.TransitionScene.extend({ctor: function (t, scene, backwards) {
    ...
    if (this._back)return cc.reverseTime(cc.pageTurn3D(this._duration, vector)); else return cc.pageTurn3D(this._duration,
        vector)
}, onEnter: function () {
    cc.TransitionScene.prototype.onEnter.call(this);
    var winSize = cc.director.getWinSize();
    ...
    var action = this.actionWithSize(cc.size(x, y));
    if (!this._back)this._outScene.runAction(cc.sequence(action, cc.CallFunc.create(this.finish, this), cc.StopGrid.create())); else {
        this._inScene.visible = false;
        this._inScene.runAction(cc.sequence(cc.show(), action, cc.callFunc(this.finish, this), cc.stopGrid()))
    }
}, _sceneOrder: function () {
