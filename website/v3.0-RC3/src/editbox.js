...
cc.DOM.methods = {setPosition: function (x, y) {
    ...
    if (this.dom && !this.dom.parentNode)if (!this.getParent())if (this.dom.id == "")cc.DOM._createEGLViewDiv(this); else this.dom.appendTo(cc.container); else cc.DOM.parentDOM(this);
...
cc.DOM.parentDOM = function (x) {
    ...
    if (p.getParent())cc.DOM.parentDOM(p); else if (p.isRunning()) {
        var eglViewDiv = cc.$("#EGLViewDiv");
        if (eglViewDiv)p.dom.appendTo(eglViewDiv); else cc.DOM._createEGLViewDiv(p)
    }
    return true
};
